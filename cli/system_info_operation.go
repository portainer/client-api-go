...
func runOperationSystemSystemInfo(cmd *cobra.Command, args []string) error {
	...
	msgStr, err := parseOperationSystemSystemInfoResult(appCli.System.SystemInfo(params, nil))
	if err != nil {
		return err
	}
	if !debug {
...
func parseOperationSystemSystemInfoResult(resp0 *system.SystemInfoOK, respErr error) (string, error) {
	if respErr != nil {
		...
		// Non schema case: warning systemInfoInternalServerError is not supported
